    ...
    >
      ...
      >
        <div className="flex flex-wrap gap-1">
          {selected.map((option) => {
            const userOption = option as any
            return (
              <div
                key={option.value}
                className={cn(
                  'flex gap-1 items-center bg-secondary text-secondary-foreground px-2 py-1 rounded-sm h-fit',
                  badgeClassName,
                )}
              >
                {userOption.src && (
                  <Avatar className="size-4">
                    <AvatarImage src={userOption.src} alt={option.label} />
          ...
          <CommandPrimitive.Input
            ...
            className={cn(
              'placeholder:text-muted-foreground/70 flex-1 bg-background outline-hidden disabled:cursor-not-allowed rounded-lg h-12',
              {
          ...
          <button
            ...
            className={cn(
              'text-muted-foreground/80 hover:text-foreground focus-visible:border-ring focus-visible:ring-ring/50 absolute top-0 end-0 flex size-9 items-center justify-center rounded-md border border-transparent transition-[color,box-shadow] outline-none focus-visible:ring-[3px]',
              (hideClearAllButton ||
                disabled ||
                selected.length < 1 ||
      ...
      <div className="relative">
        <div
          className={cn(
            'border-input absolute top-2 z-10 w-full overflow-hidden rounded-md border bg-popover',
        ...
        >
          {open && (
            <CommandList
              className="bg-popover text-popover-foreground shadow-lg outline-hidden max-h-[300px]"
